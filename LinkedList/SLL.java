...
// IMP Question in LinkedList that mostly big companies ask:
// 1. Cycle Detection
// 2. Fast-Slow Pointer
// 3. In-place Reversal of LinkedList

public class SLL {

    private Node head;
    private Node tail;
    // The purpose of taking tail variable:
    // Using tail, we can insert node at the tail in constant time
    // Otherwise it will take O(n) time
    private int size;
    ...
    private Node insertRecHelper(int val, int index, Node node) {
        ...
        node.next = insertRecHelper(val, index - 1, node.next);
        return node;
    }
